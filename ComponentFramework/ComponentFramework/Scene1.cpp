...
bool Scene1::OnCreate() {
	...
	for (int i = 0; i < 2; ++i) {
		...
		m->setMass(1.0f);
		m->setPos(Vec3(10 * i, 3, 3));
		m->setVel(Vec3(0.0f , 0.0f, 0.0f));
		...
		if(i == 0)
		m->setAngularVelocity((45.0*M_PI / 180) * Vec3(0, 1, 0));
...
void Scene1::Update(const float deltaTime) {
	//WE IMPLEMENTED THAT SO WE CAN UPDATE ACCORDINGLY
	models[0]->comparePos(models[1]->getPos(),deltaTime);
	models[1]->comparePos(models[0]->getPos(),deltaTime);
	for (Model* m : models) {
		m->Update(deltaTime);
	}
